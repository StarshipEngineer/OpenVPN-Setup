...
./build-ca < $BASE_DIR/OpenVPN-Setup/ca_info.txt
...
sed 's/LOCALIP/'$LOCALIP'/' < $BASE_DIR/OpenVPN-Setup/server_config.txt >/etc/openvpn/server.conf
...
sed 's/LOCALIP/'$LOCALIP'/' < $BASE_DIR/OpenVPN-Setup/firewall-openvpn-rules.txt >/etc/firewall-openvpn-rules.sh
...
sed 's/PUBLICIP/'$PUBLICIP'/' < $BASE_DIR/OpenVPN-Setup/Default.txt >/etc/openvpn/easy-rsa/keys/Default.txt

# Make directory under home directory for .ovpn profiles
mkdir $BASE_DIR/ovpns
chmod 777 -R $BASE_DIR/ovpns

# Make other scripts in the package executable
cd $BASE_DIR/OpenVPN-Setup
